    ...
    public ResponseEntity<List<HistoricoDisciplinaResponse>> buscarHistoricoAluno(@PathVariable Long id) {
        ...
        for (MatriculaEntity matricula : matriculas) {
            ...
            if ("CANCELADA".equals(statusMatricula)) {
                ...
                // Busca todas as avaliações da matrícula para calcular média final
                ...
                // Calcula média final se tiver prova final
                Double mediaFinal = null;
                if (mediaParcial != null && finalNota != null) {
                    mediaFinal = (mediaParcial + finalNota) / 2.0;
                }
                
                // Determina status baseado na média final
                if (mediaFinal != null) {
                    // Se tem média final, verifica aprovação/reprovação
                    if (mediaFinal >= 6.0) {
