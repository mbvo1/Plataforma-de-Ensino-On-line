...
document.getElementById('user-name').textContent = `Aluno - ${nome || 'UsuÃ¡rio'}`;
...
async function carregarPerfil() {
    ...
    try {
        const response = await fetch(`/api/aluno/${usuarioId}/perfil`, { headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token') } });
...
document.getElementById('perfil-form').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(`/api/aluno/${usuarioId}/perfil`, {
            method: 'PUT',
            headers: {
