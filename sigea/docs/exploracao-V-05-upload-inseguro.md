...
conteúdo ou tamanho de arquivo para quem quer que esteja logado"** — e,
...
![Testes via curl: upload sem auth bloqueado, upload autenticado de .html e .jsp aceitos, limite de tamanho implícito](evidencias/v05/01-testes-curl-antes-do-fix.png)
...
![Payload HTML/JS do arquivo enviado sendo executado no navegador, alterando o título da página](evidencias/v05/02-payload-html-executado-no-browser.png)
...
![Reenvio dos mesmos três ataques após o fix: todos bloqueados com 400, upload de PDF real ainda funciona](evidencias/v05/03-fix-bloqueando-os-mesmos-ataques.png)

### Evidência — código corrigido

![UploadController.java após o fix: allowlist de extensão + validação por magic bytes + nome de arquivo gerado a partir da extensão validada](evidencias/v05/04-codigo-corrigido.png)

### Evidência — aplicação corrigida rodando

![Tela de login do SIGEA servida por localhost:8080 a partir do build com o fix aplicado](evidencias/v05/05-app-rodando.png)
